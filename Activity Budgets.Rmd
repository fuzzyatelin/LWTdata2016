...
author: Laura Angley, with modifications by Christopher Schmitt
...
`Note from Chris: the input for the datafile you're using for the activity budget is missing!  This chunk is my best guess`

```{r}
pre_focals<-read.csv("2016_pre-release_FULL_UPDATED.csv",header=T)
```
...
Total:
```{r}
PreTotal_budget<- pre_focals_budget %>%
...
#This gives us all occurrences of each category:
table(PreTotal_budget$behavior.category)

# Feeding Locomote    Other Predator   Social   Stress 
#    1081      978     1016     1207      823      467 

PreTotal_budget2<- PreTotal_budget %>%
  ...
  mutate(Occurrences = #new column called "occurrrences"
           case_when(behavior.category == "Feeding" ~ as.numeric(1081), #these numbers are the number of times each category was listed in the dataframe up above (Pops_budget). I have to find these because not ever behavior category has the same number of hours it was recorded in, I need to find the unique number for each category. I found these numbers using table(Pops_budget$behavior.category) in the console
                     behavior.category == "Locomote" ~ as.numeric(978),
                     behavior.category == "Other" ~ as.numeric(1016),
                     behavior.category == "Predator" ~ as.numeric(1207),
                     behavior.category == "Social" ~ as.numeric(823),
                     behavior.category == "Stress" ~ as.numeric(467))) %>%
...
PreTotal_budget2$total<- NULL
PreTotal_budget2$Occurrences<- NULL
PreTotal_budget2$release<-rep(c("pre"),times=7)
...
pre_fullbudget<- Pops_budget2 %>%
  ...
  rename("Blue" = 2,"Amy" = 3,"Alex" = 4,"Boo" = 5,"Bgm" = 6,"Aug" = 7,"Ed" = 8,"Jack" = 9,"Tink" = 10,"Kovu" = 11,"Mango" = 12,"May" = 13,"Toni" = 14,"Nev" = 15)
...
`Note from Chris: again the dataset is missing here, so using my best guess`
```{r}
post_focals<-read.csv("2016_post-release_focals_FULL.csv",header=T)
```
...
Total:
```{r}
PostTotal_budget<- post_focals_budget %>%
  group_by(MONTH,day,hour,behavior.category) %>% #group by these columns because I need unique counts PER HOUR
  summarize(count=n()) #counts up all of the times the behavior in that category was recorded (this is per focal minute)

#This gives us all occurrences of each category:
table(PostTotal_budget$behavior.category)

# Feeding Locomote    Other Predator   Social   Stress 
#     393      381      341      429      112      143 

PostTotal_budget2<- PostTotal_budget %>%
  group_by(behavior.category) %>% #group by each behavior category
  summarize(total= sum(count)) %>% #this creates a new column called "total" that is the sum of the count column 
  mutate(Occurrences = #new column called "occurrrences"
           case_when(behavior.category == "Feeding" ~ as.numeric(393), #these numbers are the number of times each category was listed in the dataframe up above (Pops_budget). I have to find these because not ever behavior category has the same number of hours it was recorded in, I need to find the unique number for each category. I found these numbers using table(Pops_budget$behavior.category) in the console
                     behavior.category == "Locomote" ~ as.numeric(381),
                     behavior.category == "Other" ~ as.numeric(341),
                     behavior.category == "Predator" ~ as.numeric(429),
                     behavior.category == "Social" ~ as.numeric(112),
                     behavior.category == "Stress" ~ as.numeric(143))) %>%
  mutate(Mean = (total/Occurrences)) #new column called "mean" which is the total sum of each behavior category divided by the number of hours it is recorded in (i.e the occurrences column)
PostTotal_budget2$total<- NULL
PostTotal_budget2$Occurrences<- NULL
PostTotal_budget2$release<-rep(c("post"),times=7)
...
For totals:
```{r}
total_budget_table<- bind_rows(na.omit(PreTotal_budget2),na.omit(PostTotal_budget2))
total_budget_table<- total_budget_table %>%
  mutate(proportion= Mean/60)
total_budget_table$release=
factor(total_budget_table$release, levels = c("pre", "post"), ordered = TRUE)
total_budget_table

#BarChart
full_totalbudgetplot<- ggplot(data=na.omit(total_budget_table), aes(x=behavior.category, y=proportion, fill=release)) +
...
full_totalbudgetplot
```

For individual-based stats:
```{r}
...
#BarChart
full_budgetplot<- ggplot(data=na.omit(combined_budget_table), aes(x=behavior.category, y=proportion, fill=release)) +
  geom_bar(position="dodge", stat="identity") +
  scale_fill_discrete(name = "Release Stage", labels = c("Pre", "Post")) +
  scale_y_continuous(labels=scales::percent) +
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + #turning labels on an angle
...
ggpaired(fullbudget_pred,x="release",y="Means",fill="release",color="slategray4",line.color="gray",line.size = 0.4) + xlab("Release Stage") + ylab("Mean Hourly Count") + scale_fill_manual(values = c("skyblue3","skyblue3")) + theme(legend.position = "none")
```
...
ggpaired(fullbudget_social,x="release",y="Means",fill="release",color="slategray4",line.color="gray",line.size = 0.4) + xlab("Release Stage") + ylab("Mean Hourly Count") + scale_fill_manual(values = c("skyblue3","skyblue3")) + theme(legend.position = "none")
```
...
ggpaired(fullbudget_stress,x="release",y="Means",fill="release",color="slategray4",line.color="gray",line.size = 0.4) + xlab("Release Stage") + ylab("Mean Hourly Count") + scale_fill_manual(values = c("skyblue3","skyblue3")) + theme(legend.position = "none")
```
...
ggpaired(fullbudget_feed,x="release",y="Means",fill="release",color="slategray4",line.color="gray",line.size = 0.4) + xlab("Release Stage") + ylab("Mean Hourly Count") + scale_fill_manual(values = c("skyblue3","skyblue3")) + theme(legend.position = "none")
```
...
ggpaired(fullbudget_loc,x="release",y="Means",fill="release",color="slategray4",line.color="gray",line.size = 0.4) + xlab("Release Stage") + ylab("Mean Hourly Count") + scale_fill_manual(values = c("skyblue3","skyblue3")) + theme(legend.position = "none")
```
...
ggpaired(fullbudget_other,x="release",y="Means",fill="release",color="slategray4",line.color="gray",line.size = 0.4) + xlab("Release Stage") + ylab("Mean Hourly Count") + scale_fill_manual(values = c("skyblue3","skyblue3")) + theme(legend.position = "none")
```
